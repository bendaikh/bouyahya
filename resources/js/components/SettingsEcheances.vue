<template>
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-semibold text-gray-900 dark:text-white">Gestion des Échéances</h2>
            ...
            >
                ...
                <span v-if="!showForm">Nouvelle Échéance</span>
        ...
        <section v-if="showForm" class="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900">
            ...
            <div class="px-6 py-6">
                <form @submit.prevent="saveEcheance" class="space-y-4">
        ...
        <section class="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900">
            <header class="border-b border-gray-100 px-6 py-4 dark:border-gray-800">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="text-lg font-medium text-gray-900 dark:text-white">Liste des échéances</h3>
                        <p class="text-sm text-gray-500 dark:text-gray-400">
                            Consultez et gérez toutes les options d'échéance
                        </p>
                    </div>
                </div>
            </header>
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-800">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Jours</th>
                            <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Actions</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                        <tr v-for="echeance in echeances" :key="echeance.id" class="hover:bg-gray-50 dark:hover:bg-gray-800">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <button
                                        ...
                                        :disabled="removingEcheance === echeance.id"
                                        title="Supprimer"
                                    >
                                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
const removingEcheance = ref(null)
...
const loadEcheances = async () => {
    try {
        ...
        showMessage('Erreur lors du chargement des échéances', true)
    }
}
...
const saveEcheance = async () => {
    try {
        ...
        if (response.ok) {
            ...
            resetForm()
            showForm.value = false
            showMessage(editingId.value ? 'Échéance modifiée avec succès' : 'Échéance ajoutée avec succès')
        } else {
...
const editEcheance = (echeance) => {
    editingId.value = echeance.id
    formData.value = {
        code: echeance.code || '',
        libelle: echeance.libelle || '',
        jours: echeance.jours || null
    }
    showForm.value = true
}

const removeEcheance = async (echeance) => {
    if (!confirm(`Êtes-vous sûr de vouloir supprimer "${echeance.libelle || echeance.code}" ?`)) return

    try {
        removingEcheance.value = echeance.id
        const response = await fetch('/api/settings/echeances/remove', {
            ...
            body: JSON.stringify({ id: echeance.id })
        })

        const data = await response.json()
        if (response.ok) {
            echeances.value = data.echeances
            showMessage('Échéance supprimée avec succès')
        } else {
            throw new Error(data.message || 'Erreur lors de la suppression')
        }
    } catch (error) {
        console.error('Erreur:', error)
        showMessage('Erreur lors de la suppression de l\'échéance', true)
    } finally {
        removingEcheance.value = null
    }
}
